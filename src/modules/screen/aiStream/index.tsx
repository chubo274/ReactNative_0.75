...
const heightMedia = Dimensions.get('window').height
...
const AiStreamScreen = () => {
  ...
  const _renderItem = useCallback(({ item, index }: { item: any, index: number }) => {
    return <View style={{height: heightMedia - theme.dimensions.getTabBottomHeight}}>
      <ItemVideo/>
      <Video
        key={index}
        ...
        controls={false}
        style={{ width: widthMedia, height: heightMedia - theme.dimensions.getTabBottomHeight, position: 'relative', zIndex: -99 }}
        resizeMode={'cover'}
        bufferConfig={{
          minBufferMs: 2500,
          maxBufferMs: 3000,
  ...
  return  <FlatList
      ...
      snapToInterval={heightMedia - theme.dimensions.getTabBottomHeight}
      decelerationRate="fast"
      keyExtractor={(item, index) => index.toString()}
      renderItem={_renderItem}
      onMomentumScrollEnd={(event) => {
        const currentIndex = (event.nativeEvent.contentOffset.y / (heightMedia - theme.dimensions.getTabBottomHeight)).toFixed(0)
